...
# Define max_len (must match training)
max_len = 280

# Check if tweet is fluoride-related
def is_fluoride_related(tweet):
    fluoride_keywords = [
        "fluoride", "fluoridation", "water fluoridation",
        "fluorinated", "fluoride in water",
        "fluoride treatment", "fluoride toothpaste"
    ]
...
if submitted:
    if tweet_input.strip() == "":
        ...
        # FIXED LOGIC
        # If probability >= 0.5 → misinformation
        # If probability < 0.5 → safe
        if confidence >= 0.5:
            pred_class = 0   # misinformation
        else:
            pred_class = 1   # safe
